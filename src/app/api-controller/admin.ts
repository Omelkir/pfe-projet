...
export const modifier = async (req: any) => {
  try {
    ...
    const id = json.id
    const strmdp: string = json.mdp?.trim() !== '' ? ` , mdp='${json.mdp}' ` : ''
    const sql = `UPDATE medi_connect.admin SET nom_ut ='${json.nom_ut}',email ='${json.email}',image='${checkUrl}' ${strmdp} where id='${id}'`
